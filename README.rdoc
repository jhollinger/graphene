...
As you can see above, there were no log entries on 2012-07-23, so there is a gap in the X axis. That can be a problem when you're generating graphs.
There are two ways to make sure any X gaps are filled in.

    # Pass all the X points in an array, Range, or any other object implementing #to_a
    => Graphene.percentages(logs, :browser).over(:date, (Date.new(2012, 7, 22)..Date.new(2012, 7, 25)))
    # or pass a block that accepts X and returns X+1
    => Graphene.percentages(logs, :browser).over(:date) { |date| date + 1 }

Either of those will give you the following:
...
Provides helpers for generating Gruff graphs. Requires the "gruff" Ruby gem.
    ...
    Graphene.subtotals(logs, :browser).over(:date, ->(date) { date + 1 }).line_graph('/path/to/graph.png') do |chart, labeler|
